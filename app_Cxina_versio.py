...
def summarize_scores(scores):
    ...
    for r in scores:
        ...
        date_obj = None
        if ts:
            # 兼容旧数据中的宽松时间格式，并按 UTC 解释后换算为 JST 日期
...
def show_rankings(stats_data, score_rows=None):
    if is_debug_mode():
        with st.expander("Debug: Raw UserStats Data"):
            ...
            if st.button("Clear Cache & Rerun", key="clear_cache_vocab_debug_zh"):
                st.cache_data.clear()
                st.rerun()
...
def main():
    ...
    if not st.session_state.questions:
        ...
        render_cross_language_footer("vocab_zh")
        return
            ...
            if st.session_state.score_saved:
                ...
                if st.button("保存分数", key="save_score_btn", use_container_width=True):
                    ...
                    if save_score(record):
                        ...
                        st.session_state.pending_save_id = None
                        st.session_state.score_refresh_needed = True
                        st.session_state.score_sync_warning = None if stats_ok else "分数日志已保存，但累计分数同步暂时失败。请稍后重试。"
                        st.rerun()
                    else:
